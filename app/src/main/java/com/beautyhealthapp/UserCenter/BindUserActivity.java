...
import com.Entity.BindingMessage;
...
public class BindUserActivity extends DataRequestActivity implements OnClickListener{
    ...
    private void cancelBindAction() {
        ISqlHelper iSqlHelper=new SqliteHelper(null, getApplicationContext());
        List<Object> ls =iSqlHelper.Query("com.Entity.BindingMessage", null);
        if(ls.size()>0){
            ...
            if (dataResult != null) {
                ...
                if (realData.getResultcode().equals("1")) {
                    if (CurrentAction == currentNotiName) {
                        ...
                        ISqlHelper iSqlHelper=new SqliteHelper(null, getApplicationContext());
                        List<Object> ls = iSqlHelper.Query("com.Entity.BindingMessage","UnderGuardUserID = '"+msg.result+"'");
                        if(ls.size()==0){

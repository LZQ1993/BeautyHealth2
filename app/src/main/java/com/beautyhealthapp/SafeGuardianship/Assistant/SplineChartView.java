...
public class SplineChartView  extends CWChartView {
	private Context cxt;
	private List<CWChartLine2D> Lines=null;
	...
	public SplineChartView(Context context,List<CWChartLine2D> _lines,List<Integer> _colors) {
		super(context);
		cxt=context;
		Lines=_lines;
		colors=_colors;
	...
	private void setSubTitle() {
		if(Lines!=null&&Lines.size()>0){
			CWChartLine2D achartLine2d=Lines.get(0);
			List<CWChartPoint2D> chartPoints=achartLine2d.ChartPoints;
			if(chartPoints.size()>0){
				CWChartPoint2D achartPoint2d_Start=chartPoints.get(0);
				CWChartPoint2D achartPoint2d_End=chartPoints.get(chartPoints.size()-1);
				SubChartTitel=SubChartTitel+":"+achartPoint2d_Start.AXLabel+"-"+achartPoint2d_End.AXLabel;
			}
		}
	...
	private void triggerClick(float x,float y)
	{
		...
		if(LinePosition >= chartData.size()) return;
		...
		while (it.hasNext()) {
			...
			if (pos == i) {
				...
				CWChartLine2D achartLine2d = Lines.get(LinePosition);
				...
				// pos
				List<CWChartPoint2D> chartPoints = achartLine2d.ChartPoints;
				CWChartPoint2D achartPoint2d = chartPoints.get(pos);
				PointD ap = new PointD(achartPoint2d.AX, achartPoint2d.AY);
				...
				chart.getToolTip().addToolTip("  时间:" + times[1],
						mPaintTooltips);
